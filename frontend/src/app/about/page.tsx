  ...
  return (
    <>
      <div
        ...
        style={{
          backgroundImage: 'url("' + getImageURLBySize(pageData.attributes.heroImage.data, "large") + '")',
          aspectRatio: "16 / 6",
        }}
      >
      ...
      <div className="flex">
        <div className="mt-28 flex flex-col items-center justify-center gap-14 text-center md:flex-row md:text-left">
          <div className="flex w-full flex-col items-center gap-10 md:flex-row">
            <div className="sm:min-w-96 md:max-w-[30vw]">
              <Image
                width={pageData.attributes.leftImage1.data.attributes.width}
                height={pageData.attributes.leftImage1.data.attributes.height}
                alt={pageData.attributes.leftImage1.data.attributes.alternativeText ?? ""}
                className="rounded-3xl"
                src={getImageURLBySize(pageData.attributes.leftImage1.data, "large") ?? ""}
              ></Image>
              <Image
                width={pageData.attributes.leftImage2.data.attributes.width}
                height={pageData.attributes.leftImage2.data.attributes.height}
                alt={pageData.attributes.leftImage2.data.attributes.alternativeText ?? ""}
                className="-mt-14 ml-auto mr-16 hidden w-48 max-w-[20vw] rounded-2xl sm:block"
                src={getImageURLBySize(pageData.attributes.leftImage2.data, "small") ?? ""}
              ></Image>
            </div>
            <div className="flex w-full flex-col gap-5 md:w-1/2">
              <div className="font-alumniSans text-5xl font-gigabold">{pageData.attributes.title}</div>
              <div className="text-sm text-black">
                {pageData.attributes.description.split("\\n").map((paragraph) => {
                  return (
                    <>
                      {paragraph}
                      <br></br>
      ...
      <div className="mt-20 flex w-full items-center justify-center">
        <Link href={pageData.attributes.link?.data.attributes.href ?? "/categories"}>
          <Button>
